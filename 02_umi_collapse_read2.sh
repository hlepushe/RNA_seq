...
ALL_FILES=(${indir}/*.2.fastq)

# Calculate the total number of files
TOTAL_FILES=${#ALL_FILES[@]}
if [ $TOTAL_FILES -eq 0 ]; then
    echo "No .2.fastq.gz files found." >&2
    exit 1
fi
...
FS=${ALL_FILES[$((i-1))]}
FQBase=$(basename -s .2.fastq "$FS")


# Define output file paths
added_umi_file="${outdir}/${FQBase}.2.addedumi.fastq"
dedup_file="${outdir}/${FQBase}.2.dedup.fastq"
final_output="${outdir}/${FQBase}.2.dedup.woumi.fastq"
